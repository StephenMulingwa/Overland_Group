...
export function ReportBuilder({
  ...
  const downloadExcel = (table: ReportTable) => {
    ...
    XLSX.writeFile(
      wb,
      `overland_${kind}_${table.title.replace(/\s+/g, "_")}_${Date.now()}.xlsx`,
    );
  };
  ...
  const downloadAllExcel = () => {
    ...
    XLSX.writeFile(wb, `overland_${kind}_${Date.now()}.xlsx`);
  };
  ...
  const downloadPdf = async (table: ReportTable, rows: ReportRow[]) => {
    ...
    await exportSimeraReportPdf({
      ...
      fileName: `overland_${kind}_${table.title.replace(/\s+/g, "_")}_${Date.now()}.pdf`,
    });
  };
  ...
  const downloadAllPdf = async () => {
    ...
    await exportSimeraReportPdf({
      ...
      sections,
      fileName: `overland_${kind}_all_${Date.now()}.pdf`,
    });
  };
