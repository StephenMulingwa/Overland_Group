...
export async function POST(req: Request) {
  try {
    ...
    const envEmail = process.env.OVERLAND_BOOTSTRAP_EMAIL;
    const envPass = process.env.OVERLAND_BOOTSTRAP_PASSWORD;
    if (!envEmail || !envPass) {
      return NextResponse.json(
        {
          error:
            "Set OVERLAND_BOOTSTRAP_EMAIL and OVERLAND_BOOTSTRAP_PASSWORD in .env",
        },
        { status: 503 },
      );
    }
